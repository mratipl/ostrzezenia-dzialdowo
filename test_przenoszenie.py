...
def przygotuj_pamiec(minut_temu: int):
    ...
    (M.WYJSCIE / "dane.json").write_text(json.dumps({
        "zrodla": [{"id": "gios", "nazwa": "GIOŚ – jakość powietrza",
                    "ok": True, "pobrano": znacznik}],
        "zdarzenia": [],
        "stany": [{"zrodlo": "gios", "charakter": "stan", "typ": "Jakość powietrza",
                   "tytul": "Ciechanów, ul. Kraszewskiego — Dobry", "opis": "",
                   "stopien": 0, "obowiazuje_od": None, "obowiazuje_do": None}],
    }, ensure_ascii=False), encoding="utf-8")

def ile_kafelek_gios() -> int:
    dane = json.loads((M.WYJSCIE / "dane.json").read_text(encoding="utf-8"))
    return len([p for p in dane["stany"] if p["zrodlo"] == "gios"])
...
dane = json.loads((M.WYJSCIE / "dane.json").read_text(encoding="utf-8"))
gios = [z for z in dane["zrodla"] if z["id"] == "gios"][0]
print("   status źródła:", "niedostępne" if not gios["ok"] else "aktualne")
assert not gios["ok"], "źródło musi zostać oznaczone jako niedostępne"
